class AffinityAnalyzer:
    _comparison_order = [[0, 0],
                         [0, 1],
                         [1, 0],
                         [1, 1]]

    def __init__(self):
        self._top_similar = [[] for i in range(len(self._comparison_order))]

    # Получить списки похожих нейронов
    def recount_affinity(self, synapse_values, indices_elected_neuron):
        self._top_similar = [[] for i in range(len(self._comparison_order))]
    ...
    # Находит ТОП 3 схожих с заданными
    def _find_top_similar(self, synapse_values, indices_elected_neuron):
        lists_coincidence = []
        # Полное сравнение
        for i in range(len(self._comparison_order)):
            lists_coincidence.append(self._neuron_comparison(self._comparison_order[i][0], self._comparison_order[i][1],
    ...
    # Сравнить первый нейрон со всеми второй указанной сети.
    ...
    def _generate_string_response(self):
        ...
        for j in range(len(self._top_similar)):
            top_similar_str[j] += "Самый похожий в сети " + str(self._comparison_order[j][1] + 1) + ": "
