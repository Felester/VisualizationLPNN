class MainWindow():
    from tkinter import Label, Button, Tk, Canvas, Text, WORD, Frame, LabelFrame, LEFT, RIGHT, Scrollbar, Y, VERTICAL, Entry, END
    ...
    def paintingCanvas(self):
        ...
        f_bot_3.place(relwidth=0.38, relheight=0.38, relx=0.61, rely=0.6)
        ...
        self.combo_box.place(relwidth=0.98, relheight=0.1, relx=0.01, rely=0.01)

        self.Label(f_bot_1, text="Структура НС в формате (10 5):").place(relwidth=0.98, relheight=0.1, relx=0.01, rely=0.11)

        self.entry_NNstruct = self.Entry(f_bot_1, bg="white")
        self.entry_NNstruct.place(relwidth=0.98, relheight=0.1, relx=0.01, rely=0.21)

        self.Label(f_bot_1, text="Сид рандома НС(целое число)").place(relwidth=0.98, relheight=0.1, relx=0.01, rely=0.31)

        self.entry_NNseed = self.Entry(f_bot_1, bg="white")
        self.entry_NNseed.place(relwidth=0.98, relheight=0.1, relx=0.01, rely=0.41)

        self.Label(f_bot_1, text="Сид рандома для обнуления связей").place(relwidth=0.98, relheight=0.1, relx=0.01, rely=0.51)

        self.entry_zero_connection = self.Entry(f_bot_1, bg="white")
        self.entry_zero_connection.place(relwidth=0.98, relheight=0.1, relx=0.01, rely=0.61)

        self.Label(f_bot_1, text="Файл с данными для обучения").place(relwidth=0.98, relheight=0.1, relx=0.01, rely=0.71)

        self.entry_data_set = self.Entry(f_bot_1, bg="white")
        self.entry_data_set.place(relwidth=0.6, relheight=0.1, relx=0.01, rely=0.81)
        self.Button(f_bot_1, text="Открыть", command=self.open_data_file).place(relwidth=0.35, relheight=0.1, relx=0.63, rely=0.81)
        ...
        f_bot_1.place(relwidth=0.38, relheight=0.38, relx=0.01, rely=0.6)
